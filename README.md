...
> If you have created the env before August, 2025, please make sure to update the environment. It is best to just 
...
- [Run only the participant questionnaire](#run-only-the-participant-questionnaire)
...
11. [Run the dummy experiment](#run-the-dummy-experiment) to check if everything is working correctly - **IMPORTANT: For the aging subcorpus, the "Testing" mode does not work. For testing/piloting, use "Pilot" mode. For the actual data collection, use only the "Core" mode.**
